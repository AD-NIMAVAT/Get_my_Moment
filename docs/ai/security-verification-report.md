...
- **Result:** Invalid signatures are rejected with HTTP 400; duplicate webhooks are processed idempotently.
- **Classification:** VERIFIED_EXISTING
...
- **Rollback:** N/A.
- **Dependencies:** `apps/api/services/subscription_service.py`.

---

### L. Backup / Restore Security & Integrity (SEC-03)
- **Current implementation:** Production-grade backup script (`scripts/backup_database.sh`) using custom PostgreSQL format (`pg_dump -F c`), lockfile collision prevention, SHA-256 checksum generation, metadata auditing, and automated `pg_restore --list` validation. Paired with non-destructive isolated restore sandbox script (`scripts/verify_restore.sh`) and comprehensive runbook (`docs/runbooks/backup-restore.md`).
- **Verification method:** Live execution of `./scripts/backup_database.sh` and `./scripts/verify_restore.sh` on EC2 production host, creating a live backup and successfully restoring into an isolated sandbox database (`gmm_restore_test_*`).
- **Result:** Backup generated and checksum verified; isolated restore passed with 100% table and row count parity across all 9 critical tables (including `photographers`, `events`, `photos`, `faces`, `face_embeddings`, `guests`, `consents`, `admin_users`, `folders`); `pgvector` extension and `vector(128)` structure verified.
- **Classification:** FIXED & PRODUCTION VERIFIED
- **Evidence:** Live test output on EC2: 100% row count match across all 9 critical tables; automated suite `tests/security/test_backup_restore.py` PASSED.
- **Risk if failed:** Unusable or corrupt backups during disaster recovery; data loss; pgvector incompatibility.
- **Remediation:** Standardized on custom-format archives with pre-restoration checksum verification and automated sandbox verification.
- **Production impact:** Zero downtime; zero secrets exposed; high disaster-recovery confidence.
- **Required tests:** `pytest tests/security/test_backup_restore.py` and `./scripts/verify_restore.sh`.
- **Rollback:** Revert commit `1984441`.
- **Dependencies:** PostgreSQL 16, `pgvector`, Docker.
...
- **Classification:** VERIFIED_EXISTING
- **Evidence:** `curl -sI https://www.getmymoment.fun/api/v1/health` returns `nosniff`, `SAMEORIGIN`, `strict-transport-security`.
